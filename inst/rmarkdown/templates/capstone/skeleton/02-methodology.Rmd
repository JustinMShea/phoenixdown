...
When you click the **Knit** button above a document will be generated that includes both content as well as the output of any embedded code chunks within the document. You can embed a code chunk like this (`cars` is a built-in **R** dataset):
...
First load reticulate and then find what python you path resides on.
...
```{r python-setup}
use_python("ProgramData/Anaconda3/python.exe")
```

Install some packages, but create a `conda` envioronment to store them in
```{r}
